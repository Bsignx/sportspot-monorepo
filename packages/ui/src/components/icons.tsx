import { SVGProps } from 'react'
import { ChevronRight, Search, Plus } from 'lucide-react'
import { motion } from 'framer-motion'
import { Message, Show, Hide, Home, User, Lock } from 'react-iconly'

import { useBoolean } from '../hooks/use-boolean'
import { colors } from '@sportspot/tokens'

export const Icons = {
  Home,
  User,
  Lock,
  Message,
  plus: Plus,
  mySpots: {
    edit: ({ color = colors.primary, ...props }: SVGProps<SVGSVGElement>) => (
      <svg
        width="21"
        height="21"
        viewBox="0 0 21 21"
        fill="none"
        xmlns="http://www.w3.org/2000/svg"
        {...props}
      >
        <path
          d="M9.42864 3.42859H3.42864C2.97398 3.42859 2.53795 3.6092 2.21646 3.93069C1.89497 4.25218 1.71436 4.68822 1.71436 5.14287V17.1429C1.71436 17.5975 1.89497 18.0336 2.21646 18.3551C2.53795 18.6765 2.97398 18.8572 3.42864 18.8572H15.4286C15.8833 18.8572 16.3193 18.6765 16.6408 18.3551C16.9623 18.0336 17.1429 17.5975 17.1429 17.1429V11.1429"
          stroke={color}
          strokeWidth="2"
          strokeLinecap="round"
          strokeLinejoin="round"
        />
        <path
          d="M15.8569 2.14285C16.1979 1.80186 16.6604 1.61029 17.1426 1.61029C17.6249 1.61029 18.0874 1.80186 18.4284 2.14285C18.7694 2.48384 18.9609 2.94633 18.9609 3.42856C18.9609 3.9108 18.7694 4.37329 18.4284 4.71428L10.2855 12.8571L6.85693 13.7143L7.71408 10.2857L15.8569 2.14285Z"
          stroke={color}
          strokeWidth="2"
          strokeLinecap="round"
          strokeLinejoin="round"
        />
      </svg>
    ),
    delete: ({ color = colors.primary, ...props }: SVGProps<SVGSVGElement>) => (
      <svg
        width="20"
        height="20"
        viewBox="0 0 20 20"
        fill="none"
        xmlns="http://www.w3.org/2000/svg"
        {...props}
      >
        <path
          d="M2.5 5H17.5"
          stroke={color}
          strokeWidth="2"
          strokeLinecap="round"
          strokeLinejoin="round"
        />
        <path
          d="M15.8332 5V16.6667C15.8332 17.5 14.9998 18.3333 14.1665 18.3333H5.83317C4.99984 18.3333 4.1665 17.5 4.1665 16.6667V5"
          stroke={color}
          strokeWidth="2"
          strokeLinecap="round"
          strokeLinejoin="round"
        />
        <path
          d="M6.6665 5.00002V3.33335C6.6665 2.50002 7.49984 1.66669 8.33317 1.66669H11.6665C12.4998 1.66669 13.3332 2.50002 13.3332 3.33335V5.00002"
          stroke={color}
          strokeWidth="2"
          strokeLinecap="round"
          strokeLinejoin="round"
        />
        <path
          d="M8.3335 9.16669V14.1667"
          stroke={color}
          strokeWidth="2"
          strokeLinecap="round"
          strokeLinejoin="round"
        />
        <path
          d="M11.6665 9.16669V14.1667"
          stroke={color}
          strokeWidth="2"
          strokeLinecap="round"
          strokeLinejoin="round"
        />
      </svg>
    ),
  },
  visible: {
    Show,
    Hide,
  },
  logo: {
    medium: ({ ...props }: SVGProps<SVGSVGElement>) => (
      <svg
        aria-label="Sportspot Logo"
        width="128"
        height="107"
        viewBox="0 0 128 107"
        fill="none"
        xmlns="http://www.w3.org/2000/svg"
        {...props}
      >
        <ellipse cx="63.3283" cy="23.8876" rx="18.1725" ry="18.3323" fill={colors.secondary} />
        <path
          d="M56.2033 67.0021L64.1491 48.9217C77.5076 48.9076 88.371 37.9413 88.371 24.4605C88.371 10.9731 77.4936 0 64.1238 0C50.7539 0 39.8759 10.9731 39.8759 24.4612C39.8759 28.6574 40.9287 32.6085 42.7829 36.0634L56.2033 67.0021ZM64.1244 41.556C54.7797 41.556 47.1787 33.8868 47.1787 24.4612C47.1787 15.0356 54.7797 7.3671 64.1244 7.3671C73.4678 7.3671 81.0701 15.0356 81.0701 24.4612C81.0701 33.8868 73.4678 41.556 64.1244 41.556Z"
          fill={colors.primary}
        />
        <path
          d="M4.61086 88.6776C4.61086 90.5071 12.2956 89.6389 12.2956 94.4764C12.2956 97.6393 9.52911 99.4069 6.08633 99.4069C3.90386 99.4069 1.72139 98.3215 0 96.585L2.98169 93.5771C3.84238 94.6314 5.07194 95.4376 6.08633 95.5307C7.10072 95.6237 7.96142 95.1896 8.11511 94.7244C8.48398 93.5461 6.36298 93.4841 5.71746 93.298C3.28908 92.5538 0.461086 91.5305 0.461086 88.4915C0.461086 84.8634 4.21125 83.623 6.08633 83.623C8.23807 83.623 10.4205 84.6153 12.1727 86.4139L9.22172 89.3908C8.4225 88.3365 7.03924 87.4992 6.08633 87.4992C5.59451 87.4992 4.61086 87.6853 4.61086 88.6776ZM29.7673 91.5305C29.7673 96.0578 26.7856 99.4069 22.3284 99.4069C21.4062 99.4069 19.5926 98.8487 18.8856 98.1355V106.353H14.3363V83.8401H18.8856V84.9254C19.5312 84.1502 20.6992 83.623 22.021 83.623C26.4782 83.623 29.7673 87.0031 29.7673 91.5305ZM25.1871 91.5305C25.1871 89.0807 23.6194 87.0961 21.7136 87.0961C20.5455 87.0961 19.4082 87.9644 18.7934 89.2047C18.5168 89.8249 18.3016 90.6932 18.3016 91.5305C18.3016 92.3987 18.486 93.174 18.7934 93.8252C19.4389 95.0965 20.5148 95.9338 21.7136 95.9338C23.6194 95.9338 25.1871 93.9492 25.1871 91.5305ZM38.8471 87.4062C36.972 87.4062 35.4044 89.0807 35.4044 91.5305C35.4044 93.9492 36.972 95.6237 38.8471 95.6237C40.7529 95.6237 42.3206 93.9492 42.3206 91.5305C42.3206 89.0807 40.7529 87.4062 38.8471 87.4062ZM38.8471 83.623C43.3043 83.623 46.87 87.0031 46.87 91.5305C46.87 96.0578 43.3043 99.4069 38.8471 99.4069C34.4207 99.4069 30.855 96.0578 30.855 91.5305C30.855 87.0031 34.4207 83.623 38.8471 83.623ZM58.7164 84.0572L58.1016 87.4992C56.9643 87.0031 56.7491 87.0961 56.2265 87.0961C54.3822 87.0961 53.3063 88.0884 53.3063 90.5382V99.1588H48.757V83.8401H53.3063V85.2045C53.8904 84.1502 55.4888 83.623 56.5032 83.623C57.4561 83.623 58.0094 83.6541 58.7164 84.0572ZM64.6916 92.6468C64.6916 93.7631 64.6609 95.6237 66.3515 95.6237C67.1815 95.6237 67.7041 95.2826 68.2881 94.8485V98.6316C67.5811 99.0347 66.7204 99.4069 65.7982 99.4069C61.8944 99.4069 60.1422 97.7324 60.1422 92.6468V87.6233H58.4516V83.8401H60.1422V79.4368H64.6916V83.8401H68.2881V87.6233H64.6916V92.6468ZM74.9396 88.6776C74.9396 90.5071 82.6243 89.6389 82.6243 94.4764C82.6243 97.6393 79.8578 99.4069 76.4151 99.4069C74.2326 99.4069 72.0501 98.3215 70.3287 96.585L73.3104 93.5771C74.1711 94.6314 75.4007 95.4376 76.4151 95.5307C77.4295 95.6237 78.2901 95.1896 78.4438 94.7244C78.8127 93.5461 76.6917 93.4841 76.0462 93.298C73.6178 92.5538 70.7898 91.5305 70.7898 88.4915C70.7898 84.8634 74.54 83.623 76.4151 83.623C78.5668 83.623 80.7493 84.6153 82.5014 86.4139L79.5504 89.3908C78.7512 88.3365 77.368 87.4992 76.4151 87.4992C75.9232 87.4992 74.9396 87.6853 74.9396 88.6776ZM100.096 91.5305C100.096 96.0578 97.1143 99.4069 92.6571 99.4069C91.735 99.4069 89.9214 98.8487 89.2144 98.1355V106.353H84.665V83.8401H89.2144V84.9254C89.8599 84.1502 91.028 83.623 92.3497 83.623C96.8069 83.623 100.096 87.0031 100.096 91.5305ZM95.5159 91.5305C95.5159 89.0807 93.9482 87.0961 92.0423 87.0961C90.8743 87.0961 89.7369 87.9644 89.1221 89.2047C88.8455 89.8249 88.6303 90.6932 88.6303 91.5305C88.6303 92.3987 88.8148 93.174 89.1221 93.8252C89.7677 95.0965 90.8435 95.9338 92.0423 95.9338C93.9482 95.9338 95.5159 93.9492 95.5159 91.5305ZM109.176 87.4062C107.301 87.4062 105.733 89.0807 105.733 91.5305C105.733 93.9492 107.301 95.6237 109.176 95.6237C111.082 95.6237 112.649 93.9492 112.649 91.5305C112.649 89.0807 111.082 87.4062 109.176 87.4062ZM109.176 83.623C113.633 83.623 117.199 87.0031 117.199 91.5305C117.199 96.0578 113.633 99.4069 109.176 99.4069C104.749 99.4069 101.184 96.0578 101.184 91.5305C101.184 87.0031 104.749 83.623 109.176 83.623ZM124.404 92.6468C124.404 93.7631 124.373 95.6237 126.063 95.6237C126.893 95.6237 127.416 95.2826 128 94.8485V98.6316C127.293 99.0347 126.432 99.4069 125.51 99.4069C121.606 99.4069 119.854 97.7324 119.854 92.6468V87.6233H118.163V83.8401H119.854V79.4368H124.404V83.8401H128V87.6233H124.404V92.6468Z"
          fill={colors.tertiary}
        />
      </svg>
    ),
  },
  chevron: {
    right: ChevronRight,
  },
  searchBase: Search,
  search: ({ color = colors.tertiary, ...props }: SVGProps<SVGSVGElement>) => (
    <svg
      width="24"
      height="24"
      viewBox="0 0 24 24"
      fill="none"
      xmlns="http://www.w3.org/2000/svg"
      {...props}
    >
      <path
        d="M11 19C15.4183 19 19 15.4183 19 11C19 6.58172 15.4183 3 11 3C6.58172 3 3 6.58172 3 11C3 15.4183 6.58172 19 11 19Z"
        stroke={color}
        strokeWidth="1.7"
        strokeLinecap="round"
        strokeLinejoin="round"
      />
      <path
        d="M20.9999 21L16.6499 16.65"
        stroke={color}
        strokeWidth="1.7"
        strokeLinecap="round"
        strokeLinejoin="round"
      />
      <defs>
        <linearGradient
          id="paint0_linear_116_252"
          x1="12.4623"
          y1="19.2261"
          x2="26.5394"
          y2="18.8961"
          gradientUnits="userSpaceOnUse"
        >
          <stop offset="0.440392" stopColor={color} />
          <stop offset="0.747689" stopColor={color} />
          <stop offset="1" stopColor={color} />
        </linearGradient>
      </defs>
    </svg>
  ),
  heart: ({ color = colors.tertiary, ...props }: SVGProps<SVGSVGElement>) => (
    <svg
      width="24"
      height="24"
      viewBox="0 0 24 24"
      fill="none"
      xmlns="http://www.w3.org/2000/svg"
      {...props}
    >
      <path
        d="M19 14C20.49 12.54 22 10.79 22 8.5C22 7.04131 21.4205 5.64236 20.3891 4.61091C19.3576 3.57946 17.9587 3 16.5 3C14.74 3 13.5 3.5 12 5C10.5 3.5 9.26 3 7.5 3C6.04131 3 4.64236 3.57946 3.61091 4.61091C2.57946 5.64236 2 7.04131 2 8.5C2 10.8 3.5 12.55 5 14L12 21L19 14Z"
        stroke={color}
        strokeWidth="1.7"
        strokeLinecap="round"
        strokeLinejoin="round"
      />
    </svg>
  ),
  user: ({ color = colors.tertiary, ...props }: SVGProps<SVGSVGElement>) => (
    <svg
      width="25"
      height="24"
      viewBox="0 0 25 24"
      fill="none"
      xmlns="http://www.w3.org/2000/svg"
      {...props}
    >
      <path
        d="M19.7916 21V19C19.7916 17.9391 19.3526 16.9217 18.5712 16.1716C17.7898 15.4214 16.73 15 15.6249 15H9.37492C8.26985 15 7.21004 15.4214 6.42864 16.1716C5.64724 16.9217 5.20825 17.9391 5.20825 19V21"
        stroke={color}
        strokeWidth="1.7"
        strokeLinecap="round"
        strokeLinejoin="round"
      />
      <path
        d="M12.4999 11C14.8011 11 16.6666 9.20914 16.6666 7C16.6666 4.79086 14.8011 3 12.4999 3C10.1987 3 8.33325 4.79086 8.33325 7C8.33325 9.20914 10.1987 11 12.4999 11Z"
        stroke={color}
        strokeWidth="1.7"
        strokeLinecap="round"
        strokeLinejoin="round"
      />
    </svg>
  ),
  pinGlobal: ({ color = colors.tertiary, ...props }: SVGProps<SVGSVGElement>) => (
    <svg
      width="24"
      height="26"
      viewBox="0 0 24 26"
      fill="none"
      xmlns="http://www.w3.org/2000/svg"
      {...props}
    >
      <path
        d="M16.4728 23.6578L16.5215 23.6349L16.4952 23.588L15.9795 22.6693L15.9581 22.6312L15.9175 22.6473C15.5834 22.7798 15.2496 22.8976 14.9039 23.0006C15.1332 22.6592 15.3485 22.3043 15.5497 21.9497L15.5637 21.9251L15.5498 21.9004L14.9998 20.9254L14.9526 20.8417L14.9111 20.9285C14.4857 21.8195 13.9927 22.6553 13.4656 23.4176C13.1356 23.4846 12.8485 23.5042 12.5484 23.5061V18.3795C12.9773 18.4168 13.4206 18.472 13.8635 18.5615L13.9496 18.5789L13.9206 18.496C13.7833 18.1026 13.664 17.7305 13.596 17.378L13.5897 17.3453L13.557 17.3385C13.2097 17.2664 12.8784 17.2287 12.5484 17.209V13.3687H14.8188H14.8414L14.8564 13.3517C15.2138 12.9432 15.6393 12.5904 16.1159 12.3119L16.2753 12.2188H16.0906H12.5313V8.41531C13.6122 8.33471 14.8361 8.00493 16.0444 7.46172C16.5657 8.94083 16.9017 10.4007 17.0032 11.8597L17.0076 11.9223L17.0676 11.9041C17.3734 11.8114 17.6797 11.7557 18.0041 11.7184L18.0525 11.7129L18.0482 11.6644C17.9285 10.3025 17.5548 8.68357 16.9948 7.06366C17.9253 6.62648 18.7758 6.15325 19.4516 5.64369C20.8944 7.38119 21.7874 9.68015 21.8994 12.2188H20.7484H20.5585L20.7238 12.3123C21.6419 12.8316 22.3739 13.6668 22.8168 14.6702L22.8872 14.8299L22.912 14.6571C22.9982 14.0551 23.05 13.434 23.05 12.7937C23.05 6.18827 18.184 0.95 12 0.95C5.81594 0.95 0.95 6.18832 0.95 12.8125C0.95 19.4368 5.81604 24.6562 12 24.6562C13.606 24.6562 15.1086 24.2983 16.4728 23.6578ZM11.4516 8.4153V12.2375H6.96475C7.03755 10.6461 7.37267 9.05396 7.93846 7.46104C9.14615 7.98666 10.3701 8.33416 11.4516 8.4153ZM21.8773 15.4179L21.8769 15.4171C21.529 14.6391 20.9548 13.9935 20.2409 13.5756L20.2402 13.5752C19.7009 13.2716 19.0738 13.0812 18.4281 13.0812H18.4261C18.3252 13.0812 18.2192 13.0812 18.1125 13.1003C17.851 13.1196 17.6075 13.1766 17.3819 13.2523C17.1203 13.3285 16.8595 13.4424 16.6167 13.5749L16.6167 13.5748L16.6158 13.5753C16.2494 13.7847 15.9187 14.0697 15.6406 14.392C14.9963 15.114 14.6141 16.1009 14.6141 17.1812C14.6141 17.283 14.6277 17.3849 14.6424 17.4952C14.6445 17.5113 14.6467 17.5275 14.6489 17.544L14.6488 17.544L14.6494 17.5471C14.7015 17.8126 14.7882 18.1149 14.8918 18.4163L14.8922 18.4174C15.0561 18.8598 15.2544 19.3066 15.4181 19.6519C15.5 19.8246 15.5732 19.9722 15.6294 20.0814C15.6828 20.1854 15.7223 20.2578 15.7385 20.2823L16.1498 21.0302L16.1499 21.0303L16.4764 21.6302L16.4766 21.6305L16.8719 22.343L16.872 22.3431L18.3673 25.0244L18.4121 25.1047L18.4551 25.0234L20.9638 20.2809L20.964 20.2806L20.9668 20.276L20.9775 20.258C20.9869 20.2423 21.0006 20.2192 21.0181 20.1894C21.053 20.1298 21.1028 20.0437 21.1626 19.9374C21.2822 19.7247 21.4416 19.4309 21.6011 19.1064C21.7606 18.7821 21.9204 18.4263 22.0405 18.0899C22.1603 17.7546 22.2422 17.4339 22.2422 17.1812C22.2422 16.5552 22.1031 15.9486 21.8773 15.4179ZM18.731 19.1754L18.731 19.1754L18.7286 19.1758C18.6256 19.1945 18.5092 19.2125 18.4109 19.2125C17.6398 19.2125 16.9505 18.7734 16.5631 18.1304L16.5627 18.1297C16.3447 17.7821 16.2266 17.3602 16.2266 16.9C16.2266 16.0183 16.6979 15.2473 17.3871 14.8435C17.6238 14.7145 17.8584 14.6235 18.1255 14.587L18.1255 14.5872L18.1294 14.5864C18.2091 14.569 18.3065 14.5688 18.3937 14.5688C19.6004 14.5688 20.5609 15.6117 20.5609 16.8812V16.8922L20.5655 16.9021C20.5785 16.9306 20.5824 16.9601 20.5824 16.9938C20.5824 17.0108 20.5814 17.0282 20.5804 17.0471L20.5803 17.0477C20.5793 17.0655 20.5782 17.0847 20.5781 17.1042C20.5272 17.7284 20.2579 18.2597 19.8381 18.6444C19.5351 18.9198 19.149 19.1207 18.731 19.1754ZM5.28699 4.85772C6.3749 3.79079 7.66585 3.00049 9.09935 2.58466C8.43001 3.60235 7.8117 4.81807 7.34815 6.06998C6.55137 5.71075 5.8494 5.28491 5.28699 4.85772ZM5.90237 12.2375H2.08338C2.19528 9.68081 3.08832 7.39982 4.53124 5.66244C5.20696 6.17199 6.05748 6.64523 6.98799 7.08241C6.34962 8.92678 5.97662 10.7694 5.90237 12.2375ZM4.53144 19.9624C3.08832 18.2067 2.19528 15.9254 2.08338 13.3687H5.90128C5.94264 14.82 6.31584 16.6984 6.98733 18.5429C6.05717 18.98 5.20696 19.453 4.53144 19.9624ZM9.09802 23.004C7.66528 22.6051 6.37472 21.8158 5.28691 20.7665C5.88065 20.3049 6.58376 19.8778 7.38138 19.518C7.81253 20.7702 8.39951 21.9701 9.09802 23.004ZM11.4688 23.4872C11.169 23.4837 10.8678 23.4491 10.5507 23.3978C9.65691 22.1247 8.896 20.6339 8.31761 19.1043C9.37951 18.7111 10.4715 18.4756 11.4688 18.3977V23.4872ZM11.4688 17.2097C10.3879 17.2903 9.16406 17.62 7.95577 18.1632C7.38964 16.5336 7.03842 14.9067 6.98148 13.3875H11.4688V17.2097ZM11.4688 7.22744C10.4388 7.14853 9.31471 6.87817 8.28357 6.48387C8.89567 4.91915 9.69003 3.48195 10.5517 2.22617C10.8816 2.15914 11.1687 2.13953 11.4688 2.13765V7.22744ZM15.7167 6.48456C14.6854 6.8966 13.5611 7.16702 12.5313 7.22836V2.13779C12.8309 2.1413 13.132 2.17591 13.449 2.22717C14.3105 3.48289 15.1047 4.91999 15.7167 6.48456ZM16.6518 6.06998C16.1888 4.81894 15.5712 3.58733 14.9022 2.58515C16.3356 3.00199 17.6412 3.80855 18.713 4.85775C18.1506 5.28492 17.4486 5.71076 16.6518 6.06998Z"
        fill={color}
        stroke="black"
        strokeWidth="0.1"
      />
    </svg>
  ),
  addressMarker: (props: SVGProps<SVGSVGElement>) => (
    <svg
      width="13"
      height="15"
      viewBox="0 0 13 15"
      fill="none"
      xmlns="http://www.w3.org/2000/svg"
      {...props}
    >
      <path
        d="M11.6667 6.33333C11.6667 10.3333 6.33333 14.3333 6.33333 14.3333C6.33333 14.3333 1 10.3333 1 6.33333C1 4.91885 1.5619 3.56229 2.5621 2.5621C3.56229 1.5619 4.91885 1 6.33333 1C7.74782 1 9.10438 1.5619 10.1046 2.5621C11.1048 3.56229 11.6667 4.91885 11.6667 6.33333Z"
        stroke={colors.quaternary}
        strokeWidth="0.941176"
        strokeLinecap="round"
        strokeLinejoin="round"
      />
      <path
        d="M6.33325 8.33333C7.43782 8.33333 8.33325 7.4379 8.33325 6.33333C8.33325 5.22876 7.43782 4.33333 6.33325 4.33333C5.22868 4.33333 4.33325 5.22876 4.33325 6.33333C4.33325 7.4379 5.22868 8.33333 6.33325 8.33333Z"
        stroke={colors.quaternary}
        strokeWidth="0.941176"
        strokeLinecap="round"
        strokeLinejoin="round"
      />
    </svg>
  ),
  distanceMarker: (props: SVGProps<SVGSVGElement>) => (
    <svg
      width="12"
      height="12"
      viewBox="0 0 12 12"
      fill="none"
      xmlns="http://www.w3.org/2000/svg"
      {...props}
    >
      <path
        d="M1 5.73684L11 1L6.26316 11L5.21053 6.78947L1 5.73684Z"
        stroke={colors.quaternary}
        strokeWidth="0.672515"
        strokeLinecap="round"
        strokeLinejoin="round"
      />
    </svg>
  ),
  favorite: (props: SVGProps<SVGSVGElement>) => (
    <svg
      width="20"
      height="20"
      viewBox="0 0 20 20"
      fill="none"
      xmlns="http://www.w3.org/2000/svg"
      {...props}
    >
      <path
        d="M15.8334 11.6667C17.0751 10.45 18.3334 8.99167 18.3334 7.08333C18.3334 5.86776 17.8505 4.70197 16.991 3.84243C16.1314 2.98289 14.9657 2.5 13.7501 2.5C12.2834 2.5 11.2501 2.91667 10.0001 4.16667C8.75008 2.91667 7.71675 2.5 6.25008 2.5C5.03451 2.5 3.86872 2.98289 3.00918 3.84243C2.14963 4.70197 1.66675 5.86776 1.66675 7.08333C1.66675 9 2.91675 10.4583 4.16675 11.6667L10.0001 17.5L15.8334 11.6667Z"
        stroke={colors.primary}
        strokeWidth="1.08571"
        strokeLinecap="round"
        strokeLinejoin="round"
      />
    </svg>
  ),
  favoriteFilled: (props: SVGProps<SVGSVGElement>) => (
    <svg
      width="20"
      height="20"
      viewBox="0 0 20 20"
      fill="none"
      xmlns="http://www.w3.org/2000/svg"
      {...props}
    >
      <path
        d="M15.8334 11.6667C17.0751 10.45 18.3334 8.99167 18.3334 7.08333C18.3334 5.86776 17.8505 4.70197 16.991 3.84243C16.1314 2.98289 14.9657 2.5 13.7501 2.5C12.2834 2.5 11.2501 2.91667 10.0001 4.16667C8.75008 2.91667 7.71675 2.5 6.25008 2.5C5.03451 2.5 3.86872 2.98289 3.00918 3.84243C2.14963 4.70197 1.66675 5.86776 1.66675 7.08333C1.66675 9 2.91675 10.4583 4.16675 11.6667L10.0001 17.5L15.8334 11.6667Z"
        fill={colors.primary}
        stroke={colors.primary}
        strokeWidth="1.08571"
        strokeLinecap="round"
        strokeLinejoin="round"
      />
    </svg>
  ),
  Logout: (props: SVGProps<SVGSVGElement>) => (
    <svg
      width="24"
      height="24"
      viewBox="0 0 24 24"
      fill="none"
      xmlns="http://www.w3.org/2000/svg"
      {...props}
    >
      <path
        d="M9 21H5C4.46957 21 3.96086 20.7893 3.58579 20.4142C3.21071 20.0391 3 19.5304 3 19V5C3 4.46957 3.21071 3.96086 3.58579 3.58579C3.96086 3.21071 4.46957 3 5 3H9"
        stroke={colors.secondary}
        strokeWidth="2"
        strokeLinecap="round"
        strokeLinejoin="round"
      />
      <path
        d="M16 17L21 12L16 7"
        stroke={colors.secondary}
        strokeWidth="2"
        strokeLinecap="round"
        strokeLinejoin="round"
      />
      <path
        d="M21 12H9"
        stroke={colors.secondary}
        strokeWidth="2"
        strokeLinecap="round"
        strokeLinejoin="round"
      />
    </svg>
  ),
  ratingStar: ({ color = '#E8EB5F', ...props }: SVGProps<SVGSVGElement>) => (
    <svg
      width="14"
      height="14"
      viewBox="0 0 14 14"
      fill="none"
      xmlns="http://www.w3.org/2000/svg"
      {...props}
    >
      <path
        d="M6.38624 5.3816L7 3.90594L7.61376 5.3816C7.90178 6.07407 8.553 6.54721 9.30058 6.60715L10.8937 6.73487L9.67991 7.7746C9.11033 8.2625 8.86158 9.02805 9.0356 9.75756L9.40643 11.3122L8.04251 10.4791C7.40248 10.0882 6.59753 10.0882 5.95749 10.4791L4.59357 11.3122L4.9644 9.75757C5.13842 9.02805 4.88967 8.2625 4.32009 7.7746L3.10632 6.73487L4.69942 6.60715C5.447 6.54721 6.09822 6.07408 6.38624 5.3816Z"
        fill={color}
        stroke={color}
        strokeWidth="3"
      />
    </svg>
  ),
  leftArrow: (props: SVGProps<SVGSVGElement>) => (
    <svg
      width="13"
      height="22"
      viewBox="0 0 13 22"
      fill="none"
      xmlns="http://www.w3.org/2000/svg"
      {...props}
    >
      <path
        d="M11 20L2 11L11 2"
        stroke={colors.primary}
        strokeWidth="3"
        strokeLinecap="round"
        strokeLinejoin="round"
      />
    </svg>
  ),
  motionIcons: {
    Facebook: () => {
      const [isAnimate, setAnimate] = useBoolean()

      return (
        <motion.svg
          width="50"
          height="50"
          viewBox="0 0 50 50"
          fill="none"
          xmlns="http://www.w3.org/2000/svg"
          onHoverStart={setAnimate.on}
          onHoverEnd={setAnimate.off}
        >
          <rect
            x="0.4"
            y="0.4"
            width="49.2"
            height="49.2"
            rx="13.6"
            stroke={isAnimate ? '#d9e0ea' : '#DDDADA'}
            strokeWidth="0.8"
          />

          {isAnimate && (
            <motion.rect
              x="0.4"
              y="0.4"
              width="49.2"
              height="49.2"
              rx="13.6"
              stroke="#a3b3cd"
              strokeWidth="1"
              initial={{ strokeDasharray: '0 100 80 0', strokeOpacity: 0 }}
              animate={{ strokeDasharray: '100 0', strokeOpacity: 1 }}
              transition={{ duration: 1, ease: 'linear' }}
            />
          )}
          <path
            d="M30 16.6667H27.5C26.395 16.6667 25.3352 17.1057 24.5538 17.8871C23.7724 18.6685 23.3334 19.7283 23.3334 20.8333V23.3333H20.8334V26.6667H23.3334V33.3333H26.6667V26.6667H29.1667L30 23.3333H26.6667V20.8333C26.6667 20.6123 26.7545 20.4004 26.9108 20.2441C27.0671 20.0878 27.279 20 27.5 20H30V16.6667Z"
            fill="#1877F2"
          />
        </motion.svg>
      )
    },
    Google: () => {
      const [isAnimate, setAnimate] = useBoolean()

      return (
        <motion.svg
          width="50"
          height="50"
          viewBox="0 0 50 50"
          onHoverStart={setAnimate.on}
          onHoverEnd={setAnimate.off}
          fill="none"
          xmlns="http://www.w3.org/2000/svg"
        >
          <rect
            x="0.4"
            y="0.4"
            width="49.2"
            height="49.2"
            rx="13.6"
            stroke={isAnimate ? colors.primary : colors.tertiary}
            strokeWidth="1"
          />
          {isAnimate && (
            <motion.rect
              x="0.4"
              y="0.4"
              width="49.2"
              height="49.2"
              rx="13.6"
              stroke={colors.primary}
              strokeWidth="1"
              initial={{ strokeDasharray: '0 100 80 0', strokeOpacity: 0 }}
              animate={{ strokeDasharray: '100 0', strokeOpacity: 1 }}
              transition={{ duration: 1, ease: 'linear' }}
            />
          )}

          <g clipPath="url(#clip0_401_160)">
            <path d="M35 15H15V35H35V15Z" fill="url(#pattern0)" />
          </g>

          <defs>
            <pattern id="pattern0" patternContentUnits="objectBoundingBox" width="1" height="1">
              <use xlinkHref="#image0_401_160" transform="scale(0.000976562)" />
            </pattern>
            <clipPath id="clip0_401_160">
              <rect width="20" height="20" fill="white" transform="translate(15 15)" />
            </clipPath>
            <image
              id="image0_401_160"
              width="1024"
              height="1024"
              xlinkHref="data:image/png;base64,"
            />
          </defs>
        </motion.svg>
      )
    },
  },
}
